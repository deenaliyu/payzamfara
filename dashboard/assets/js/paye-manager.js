...
$("#payeID").html(userInfo2?.tax_number)
let AllEmployees;
...
async function fetchPayeUser() {
  const response = await fetch(`${HOST}/?getSpecialUsers&id=${userInfo2?.tax_number}`)
  ...
  if (specialUsers.status === 0) {
    ...
    $("#total_remi").html(theInfo.total_remittance ? formatMoney(parseFloat(theInfo.total_remittance)) : formatMoney(0))
    $("#reg_staffs").html(theInfo.staff_count)
  }
...
async function getStaffLists() {
  ...
  const response = await fetch(`${HOST}/?getSpecialUsersEmplyees&payer_id=${userInfo2?.tax_number}`)
  const specialUsers = await response.json()
  ...
  table2 = $('#dataTableGG').DataTable({
    ...
    ajax: function (data, callback, settings) {
      ...
      const filters = {
        ...
        tax_number: userInfo2?.tax_number,
      };
    ...
    columns: [
      ...
      { data: 'time_in' },
      {
        data: null,
        render: function (data, type, row) {
          return `<button class="button" onclick="fetchTheStaffs2('${row.id}')">Staffs</button>`
        }
      }
    ],
  });
...
async function registerEmployeesInvoice(amount, invoice_num, staff_id) {
  let dataToSend = {
    ...
    data: {
      ...
      staff_id: staff_id,
      associated_special_user_id: userInfo2?.tax_number,
...
async function getPaymentHistory() {
  ...
  const response = await fetch(`${HOST}/?getSpecialUsersPayments&offset=0&payer_id=${userInfo2?.tax_number}`)
  const specialUsers = await response.json()
...
async function getInvoiceHistory() {
  ...
  const response = await fetch(`${HOST}/?userInvoices&payer_id=${userInfo2?.tax_number}`)
  const specialUsers = await response.json()
